...
<template>
  <aside :class="[
    'flex w-56 sm:w-60 lg:w-64 shrink-0 flex-col bg-white dark:bg-[#151515] border-r border-slate-200 dark:border-white/5 overflow-hidden shadow-2xl z-20 h-full duration-300 ease-in-out',
    isSidebarOpen ? 'translate-x-0 ml-0 transition-[margin,transform]' : '-translate-x-full -ml-56 sm:-ml-60 lg:-ml-64 transition-[margin,transform]'
  ]">
    ...
    <div class="flex-1 overflow-y-auto sidebar-scroll py-2 px-2 space-y-0.5">
      ...
      >
        <a :href="href" @click="navigate" :class="[
          ...
          <Icon :icon="item.meta.icon" class="text-[20px] transition-transform group-hover:scale-110" />
          {{ item.meta.title }}
        </a>
      </router-link>
